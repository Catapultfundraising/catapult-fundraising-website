...
// Sample weekly status report for the anonymized study dashboard at
// /decks/sample-dashboard. Identical layout to the live client report
// (app/api/jag-summary-pdf) with the sample dataset instead of live data.
...
const CLIENT_NAME = SAMPLE_ORG_NAME;
...
export async function GET() {
  ...
  // Same fixed sample dataset the /decks/sample-dashboard page renders, so
  // the PDF a prospect downloads always matches the numbers on screen.
  ...
  const doc = (
    <Document>
      <Page size={[PAGE_W, PAGE_H]} style={styles.page}>
        ...
        <Text style={styles.eyebrow}>{CLIENT_NAME} · Feasibility Study</Text>
  ...
  return new NextResponse(new Uint8Array(buffer), {
    ...
    headers: {
      ...
      "Content-Disposition": 'attachment; filename="Sample_Feasibility_Study_Weekly_Summary.pdf"',
    },
  });
}
