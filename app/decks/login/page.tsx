...
function DecksLoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/decks-login", {
        method: "POST",
    ...
    <div className="flex min-h-[70vh] items-center justify-center px-6 py-24">
      <div className="w-full max-w-md rounded-2xl border border-[rgb(var(--line))] bg-white p-8 shadow-sm">
        ...
        <p className="mt-2 text-center text-sm text-[rgb(var(--ink))]/65">
          Catapult sales decks, for team and invited prospects. Enter the shared password to continue.
        </p>
...
export default function DecksLoginPage() {
  return (
