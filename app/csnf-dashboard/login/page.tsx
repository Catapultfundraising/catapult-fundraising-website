...
function CsnfLoginForm() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const redirectTo = searchParams.get("redirect") || "/csnf-dashboard";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/csnf-login", {
        method: "POST",
    ...
    <div className="flex min-h-[70vh] items-center justify-center px-6 py-24">
      <div className="w-full max-w-md rounded-2xl border border-[rgb(var(--line))] bg-white p-8 shadow-sm">
        ...
        <p className="mt-2 text-center text-sm text-[rgb(var(--ink))]/65">
          This page is for authorized team members only. Enter the shared password to continue.
        </p>
...
export default function CsnfLoginPage() {
  return (
