...
async function sendEmailNotification(fields: Fields) {
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      subject: `Gift chart calculator: ${fields.name}${fields.org ? ` (${fields.org})` : ""} - ${formatDollars(fields.goal)} goal`,
      html,
    }),
  });

  if (!res.ok) {
    console.error("Resend API error (gift chart lead):", res.status, await res.text());
    return { sent: false };
  }
  return { sent: true };
}
