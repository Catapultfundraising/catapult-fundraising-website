...
function askCalloutBox(strategy: AskStrategy): Table {
  // Uses the same navy blue as every other shaded table in this document
  // (and on catapultfr.com itself) so the callout matches the site's blue
  // rather than introducing a separate one-off color.
  const border = { style: BorderStyle.SINGLE, size: 6, color: BRASS };
        ...
        children: [
          new TableCell({
            ...
            children: [
              new Paragraph({
                children: [
                  new TextRun({ text: "RECOMMENDED ASK", color: BRASS_LIGHT, bold: true, size: 16 }),
                ],
              }),
              new Paragraph({
                spacing: { before: 60 },
