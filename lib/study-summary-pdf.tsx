...
// The PDF is shared by every study portal; the calling route fixes the client.
...
// Each client's PDF sits behind that client's own password gate, so the portal
// is fixed by the route that is called and never taken from a query parameter.
// A JAG login must not be able to ask this endpoint for the CSNF report.
export async function buildSummaryPdf(slug: string) {
