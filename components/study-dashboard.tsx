...
import {
  ...
  type JagDashboardData,
} from "@/lib/jag-data";

// ---------------------------------------------------------------------------
// Shared client study dashboard. Originally the JAG Nevada page; lifted into a
// component so every calling program synced from VanillaSoft (CSNF next) gets
// the identical tracker without the layout being rebuilt each time.
// ---------------------------------------------------------------------------
...
export interface StudyDashboardProps {
  ...
  title: string;
  description: string;
  /** Link to a printable summary, when the portal has one. */
  pdfPath?: string;
}
...
export function StudyDashboard({ data, eyebrow, title, description, pdfPath }: StudyDashboardProps) {
